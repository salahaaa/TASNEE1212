...
public class PlanClosureService : ServiceBase, IPlanClosureService
{
    ...
    public PlanClosureInfo GetInfo(int planId)
    {
        ...
        foreach (var g in info.Orders.Where(o => !o.IsCancelled).GroupBy(o => o.CustomerName))
            info.Customers.Add(new ClosureSummaryRow
            {
                Name = g.Key,
                Planned = g.Sum(x => x.Planned),
                Produced = g.Sum(x => x.Produced),
                Closed = g.Sum(x => x.Closed),
                Remaining = Math.Max(0, g.Sum(x => x.Planned) - g.Sum(x => x.Produced)),
                StateAr = g.All(x => x.StateAr == "مقفل") ? "مقفل ✔" : "غير مكتمل"
            });
