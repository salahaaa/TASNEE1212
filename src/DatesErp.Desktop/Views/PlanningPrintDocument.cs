...
public static class PlanningPrintDocument
{
    ...
    private static Block ItemsTable(PlanningPrintModel m)
    {
        ...
        foreach (var it in m.Items) // مرتبة بالتاريخ من النموذج
        {
            ...
            if (isNewDay)
                r.Cells.Add(new TableCell(new Paragraph(new Run("تاريخ التشغيل: " + it.Date)) { TextAlignment = TextAlignment.Center }) { Padding = new Thickness(2, 3, 2, 3), ColumnSpan = 10, FontSize = 9 });
            t.RowGroups[0].Rows.Add(r);
