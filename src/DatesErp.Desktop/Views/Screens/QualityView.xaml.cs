    ...
    private void Save()
    {
        try
        {
            ...
            string balLine = summary.ProducedQty == null ? ""
                ...
                : $"⚠️ النتائج ({UiFormat.N(summary.TotalQty)}) لا تغطي الإنتاج ({UiFormat.N(summary.ProducedQty)}) — المحضر «قيد الفحص» حتى الاكتمال.\n";
    ...
    private void Print()
    {
        ...
        try
        {
            ...
            foreach (var u in t.ByUnit)
            {
                ...
                if (u.NonConforming > 0) m.Totals.Add((($"غير مطابق / منسم ({u.UnitLabel})"), UiFormat.N(u.NonConforming)));
