...
public partial class DeliveryView : UserControl
{
        ...
        try
        {
            ...
            foreach (var b in rows)
            {
                _balances.Add(new DelivBalanceRow
                {
                    ...
                    ProductName = db.Products.Where(p => p.Id == b.ProductId).Select(p => p.ProductNameAr).FirstOrDefault() ?? "-",
                    LotCode = db.Lots.Where(l => l.Id == b.LotId).Select(l => l.LotCode).FirstOrDefault() ?? "—",
                    Qty = b.QtyKg,
                    Packages = b.PackageCount,
                    UnitWeight = b.PackageCount > 0 ? Math.Round(b.QtyKg / b.PackageCount, 3) : (db.Products.Where(p => p.Id == b.ProductId).Select(p => p.CartonWeightKg).FirstOrDefault() is double cw && cw > 0 ? cw : 7.5)
                });
            }
    ...
    private void DeliverAll_Click(object sender, RoutedEventArgs e)
    {
        if (_locked) { AppContainer.Get<DialogService>().Error("السند مقفل (معتمد)."); return; }
        _items.Clear();
        foreach (var b in _balances.Where(b => b.Qty > 0.001))
        {
            _items.Add(new DelivBalanceRow
            {
    ...
    private void Save()
    {
        try
        {
            ...
            var r = svc.Save(_currentCustomerId, (DateBox.SelectedDate ?? DateTime.Now).ToString("dd/MM/yyyy"), orderId,
                _items.Select(i => new CustomerDeliveryItemDto
                {
            ...
            AppContainer.Get<DialogService>().Info($"تم حفظ سند التسليم رقم: {r.DocumentNumber}\nالسند باقٍ أمامك — اعتمده لخصم الكميات من رصيد العميل.");
            RefreshList();
        }
