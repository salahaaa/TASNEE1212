...
public class PlanCheckB91Tests
{
    ...
    {
        using var (host, a, b) = Seed2Customers();
    ...
    public void Overload_Shortage_Detected_With_Short_Days()
    {
        ...
        {
            new() { CustomerId = a, ProductId = 3, PackagingTypeId = pack, PlannedCartons = 100000, PlannedQtyKg = 500000 },
        });
        Assert.True(r.Ok);

        var c = Plan(host).CheckPlan(r.Id);
        Assert.False(c.Ok);
        Assert.True(c.ShortageKg > 0);
        Assert.Equal(95000, c.CoveredKg); // 47500 × يومين
        Assert.All(c.Days, d => Assert.Equal("Short", d.Status));
